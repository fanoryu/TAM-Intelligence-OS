...
| [UX-006A-Identity-Foundation-Implementation-Plan.md](UX-006A-Identity-Foundation-Implementation-Plan.md) | UX-006A **implementation plan** (NOT IMPLEMENTED): exact file boundaries, `User` contract, `IdentityProvider` seam, single `currentUser` selector, CEO + Employee representative principals, fail-closed matrix, no-persistence decision (SCHEMA_VERSION stays 6), test/verifier plan, implementation order, acceptance criteria, and stop conditions. Translates the frozen UX-006 baseline into a mechanically-executable build plan; awaiting owner review before an implementation assignment |
